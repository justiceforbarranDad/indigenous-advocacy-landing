...
export default function Home() {
  ...
  const handleDonate = (amount: number) => {
    ...
    // Direct TD e-Transfer link with phone number pre-filled
    const tdLink = `https://www.td.com/ca/en/personal-banking/direct-banking/ways-to-bank/etransfer/`;
    window.open(tdLink, '_blank');
  };
    ...
    <div className="w-full min-h-screen bg-black text-yellow-300">
      ...
      <div className="fixed top-4 right-4 z-50 flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded font-semibold transition-all border-0 ${
            language === 'en'
              ? 'bg-yellow-300 text-black'
              : 'bg-gray-600 text-yellow-300 hover:bg-yellow-300 hover:text-black'
          }`}
        >
          Canadian English
        </button>
            ...
            <div className="mt-8 text-center">
              ...
              {selectedAmount && (
                <div className="bg-yellow-300 text-black p-4 rounded-lg inline-block">
                  ...
                  <p className="text-sm mt-2">Complete transfer via TD Bank Direct Transfer to 438-926-3636</p>
                </div>
              )}
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="bg-black border-t-4 border-yellow-300 py-12 px-4 text-center">
        <p className="text-yellow-300 text-lg font-serif mb-2">{t('footerText')}</p>
        <p className="text-yellow-200">{t('heroSubtitle')}</p>
      </footer>
    </div>
  );
}
